...
// note: struct insolStats is defined in const.h
...
// data written to CSV file; strucure: date , consumed power (regulated, accumulated, manual), insolation Wh per hourly timeslot, span = 04.00 - 19.00 hour, total insolation (Wh/d)
    ...
    if (file) {
      sprintf(buff, "%d-%02d-%02d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%04d;%05d", // we use 121 char
...
int insolStatsEvalCurrentInsolPower() { 
      return insolPower; // calibrated insolation power
  }
...
int insolStatsInsolationPowerMonth() { // this is used by display and webserver
  if (day(now()) != day(insolData.timestamp))
    return 0;
  return insolData.month.insolationPower; // Wh
}
