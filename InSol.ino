// ** The insolation value is read from the shortcut current of a 1W "reference" PV panel **
...
// maximum normal surface irradiance is approximately 1000 W/m2 at sea level on a clear day

// Solar Panel: Voltaic Systems 6V 1,12W  (0.20A expected Isc shortcut current), shorted with a 15Ω 5W resister, and connected to ADC

float Isc = 0.20; // shortcut current in Ampere
float Rsc = 15;  // shortcut resistance in Ohm –> at 0,21A max 3 volt on ADC input (R=V/I)
...
float IRref = 1.5000; // correction of Irradiance value with help of external reference

volatile float CurrentRatio; // ratio of measured versus maximum current
...
// Power calculations
  ...
  CurrentRatio = Ix / Isc; // the factor of measured versus short cut current, max = 1000W per square meter

  insolPower = CurrentRatio * IRref; // correction for Irradiance calibration value
